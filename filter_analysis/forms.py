...
from filter_analysis.models import FilterAnalysis, FamilyFilterAnalysis
from genes.models import GeneGroup, GeneList, CGDCondition, Manifestation
...
class FilterAnalysisForm(forms.Form):
    ...
    # individuals = forms.MultipleChoiceField(choices=[(x.id, x.name)
    # for x in Individual.objects.filter().order_by('id')], required=False)
    ...
    # individuals = forms.ModelMultipleChoiceField(queryset=Individual.objects.all().order_by('id'),
    # required=False, label='INDIVIDUALS')

    exclude_individuals = forms.ModelMultipleChoiceField(queryset=Individual.objects.all().order_by('id'),
                                                         required=False, label='EXCLUDE INDIVIDUALS')

    groups = forms.ModelMultipleChoiceField(queryset=Group.objects.all().order_by('id'), required=False, label='GROUPS')
    exclude_groups = forms.ModelMultipleChoiceField(queryset=Group.objects.all().order_by('id'),
                                                    required=False, label='EXCLUDE GROUPS')
    ...
    genes_in_common = forms.BooleanField(required=False,
                                         label="SHOW ONLY VARIANTS PRESENT IN COMMON GENES BETWEEN ALL THE "
                                               "INDIVIDUALS SELECTED")
    positions_in_common = forms.BooleanField(required=False, label="SHOW ONLY VARIANTS AT EXACTLY SAME POSITION "
                                                                   "BETWEEN ALL THE INDIVIDUALS SELECTED")
    ...
    CHOICES = [(x, x.replace('_', ' ')) for x in [
        'CDS', 'CHROMOSOME_LARGE DELETION', 'CODON_CHANGE', 'CODON_INSERTION', 'CODON_CHANGE_PLUS CODON_INSERTION',
        'CODON_DELETION', 'CODON_CHANGE_PLUS CODON_DELETION', 'DOWNSTREAM', 'EXON', 'EXON_DELETED', 'FRAME_SHIFT',
        'GENE', 'INTERGENIC', 'INTERGENIC_CONSERVED', 'INTRAGENIC', 'INTRON', 'INTRON_CONSERVED', 'MICRO_RNA',
        'NON_SYNONYMOUS_CODING', 'NON_SYNONYMOUS_START', 'NON_SYNONYMOUS_STOP', 'RARE_AMINO_ACID',
        'SPLICE_SITE_ACCEPTOR', 'SPLICE_SITE_DONOR', 'SPLICE_SITE_REGION', 'SPLICE_SITE_BRANCH',
        'SPLICE_SITE_BRANCH_U12', 'STOP_LOST', 'START_GAINED', 'START_LOST', 'STOP_GAINED', 'SYNONYMOUS_CODING',
        'SYNONYMOUS_START', 'SYNONYMOUS_STOP', 'TRANSCRIPT', 'REGULATION', 'UPSTREAM', 'UTR_3_PRIME', 'UTR_3_DELETED',
        'UTR_5_PRIME', 'UTR_5_DELETED']]

    # use SnpeffAnnotation
    # (x[0], x[0].replace('_', ' '))
    # for x in Variant.objects.values_list('snpeff_effect').distinct().order_by('snpeff_effect')

    effect = forms.MultipleChoiceField(choices=CHOICES, required=False)
    # effect = forms.MultipleChoiceField(choices=[(x[0], x[0])
    # for x in Variant.objects.values_list('snpeff_effect').distinct().order_by('snpeff_effect')], required=False)

    CHOICES = [(x, x.replace('_', ' ')) for x in ['NONE', 'SILENT', 'MISSENSE', 'NONSENSE']]

    # [(x[0], x[0]) for x in Variant.objects.values_list('snpeff_functional_class').distinct().exclude(
    # snpeff_functional_class='').order_by('snpeff_functional_class')]
    ...
    # filter = forms.MultipleChoiceField(choices=[(x[0], x[0])
    # for x in Variant.objects.values_list('filter').distinct().order_by('filter')], required=False)
    ...
    # SIFT
    # ljb_sift_class = forms.ChoiceField(choices=[(x[0], x[0]) for x in Variant.objects.values_list('ljb_sift_pred')
    # .distinct().exclude(ljb_sift_pred='').order_by('ljb_sift_pred')], required=False)
    ...
    # diseases
    # diseases = forms.MultipleChoiceField(choices=[(x[0], x[0])
    # for x in Disease.objects.values_list('name').distinct()], required=False)

    fields = forms.MultipleChoiceField(choices=FIELDS, required=False)

    # conditions = forms.ModelMultipleChoiceField(queryset=CGDCondition.objects.filter()
    ...
    genelists = forms.ModelMultipleChoiceField(queryset=GeneList.objects.all().order_by('name'), required=False,
                                               label="SAVED GENE LIST")
    exclude_genelists = forms.ModelMultipleChoiceField(queryset=GeneList.objects.all().order_by('name'), required=False,
                                                       label="EXCLUDE SAVED GENE LIST")
    ...
    def __init__(self, user=None, *args, **kwargs):
        ...
        if not user.is_authenticated:
            ...
            self.fields['individuals'] = forms.ModelMultipleChoiceField(
                queryset=Individual.objects.filter(user=None).order_by('id'), required=False, label='INDIVIDUALS')
            ...
            if user.is_superuser:
                self.fields['individuals'] = forms.ModelMultipleChoiceField(
                    queryset=Individual.objects.filter().order_by('id'), required=False, label='INDIVIDUALS')
                self.fields['exclude_individuals'] = forms.ModelMultipleChoiceField(
                    queryset=Individual.objects.filter().order_by('id'), required=False, label='INDIVIDUALS')
            else:
                self.fields['individuals'] = forms.ModelMultipleChoiceField(
                    queryset=Individual.objects.filter(Q(user=user) | Q(user=None)).order_by('id'),
                    required=False, label='INDIVIDUALS')
                self.fields['exclude_individuals'] = forms.ModelMultipleChoiceField(
                    queryset=Individual.objects.filter(Q(user=user) | Q(user=None)).order_by('id'),
                    required=False, label='INDIVIDUALS')
...
class FamilyAnalysisForm(forms.Form):
    ...
    genes_in_common = forms.BooleanField(required=False,
                                         label="SHOW ONLY VARIANTS PRESENT IN COMMON GENES BETWEEN ALL THE INDIVIDUALS SELECTED")
    positions_in_common = forms.BooleanField(required=False,
                                             label="SHOW ONLY VARIANTS AT EXACTLY SAME POSITION BETWEEN ALL THE INDIVIDUALS SELECTED")
    is_at_hgmd = forms.BooleanField(required=False, label="SHOW ONLY VARIANTS PRESENT AT HGMD")
    exclude_varisnp = forms.BooleanField(required=False, label="EXCLUDE VARIANTS AT VARISNP")
    ...
    CHOICES = [(x, x.replace('_', ' ')) for x in ['CDS', 'CHROMOSOME_LARGE DELETION', 'CODON_CHANGE', 'CODON_INSERTION',
                                                  'CODON_CHANGE_PLUS CODON_INSERTION', 'CODON_DELETION',
                                                  'CODON_CHANGE_PLUS CODON_DELETION', 'DOWNSTREAM', 'EXON',
                                                  'EXON_DELETED', 'FRAME_SHIFT', 'GENE', 'INTERGENIC',
                                                  'INTERGENIC_CONSERVED', 'INTRAGENIC', 'INTRON', 'INTRON_CONSERVED',
                                                  'MICRO_RNA', 'NON_SYNONYMOUS_CODING', 'NON_SYNONYMOUS_START',
                                                  'NON_SYNONYMOUS_STOP', 'RARE_AMINO_ACID', 'SPLICE_SITE_ACCEPTOR',
                                                  'SPLICE_SITE_DONOR', 'SPLICE_SITE_REGION', 'SPLICE_SITE_BRANCH',
                                                  'SPLICE_SITE_BRANCH_U12', 'STOP_LOST', 'START_GAINED', 'START_LOST',
                                                  'STOP_GAINED', 'SYNONYMOUS_CODING', 'SYNONYMOUS_START',
                                                  'SYNONYMOUS_STOP', 'TRANSCRIPT', 'REGULATION', 'UPSTREAM',
                                                  'UTR_3_PRIME', 'UTR_3_DELETED', 'UTR_5_PRIME', 'UTR_5_DELETED']]
    ...
    INHERITANCE_CHOICES = (('', ''),
                           ...
                           ('xlinked dominant', 'X-linked Dominant'),
                           # ('recessive denovo', 'Autsomal Recessive denovo'),
    ...
    def __init__(self, user=None, *args, **kwargs):
        ...
        if not user.is_authenticated:
            # print('user None', user)
            self.fields['mother'] = forms.ModelMultipleChoiceField(
                queryset=Individual.objects.filter(user=None).order_by('id'), required=False)
            self.fields['father'] = forms.ModelMultipleChoiceField(
                queryset=Individual.objects.filter(user=None).order_by('id'), required=False)
...
class FilterWiZardForm1(forms.Form):
    individuals = forms.ModelMultipleChoiceField(queryset=Individual.objects.all().order_by('id'),
                                                 required=False)  # choices=INDIVIDUALS,
...
class FilterWiZardForm2(forms.Form):
    ...
    CHOICES = ['CDS', 'CHROMOSOME_LARGE DELETION', 'CODON_CHANGE', 'CODON_INSERTION',
               'CODON_CHANGE_PLUS CODON_INSERTION', 'CODON_DELETION', 'CODON_CHANGE_PLUS CODON_DELETION', 'DOWNSTREAM',
               'EXON', 'EXON_DELETED', 'FRAME_SHIFT', 'GENE', 'INTERGENIC', 'INTERGENIC_CONSERVED', 'INTRAGENIC',
               'INTRON', 'INTRON_CONSERVED', 'MICRO_RNA', 'NON_SYNONYMOUS_CODING', 'NON_SYNONYMOUS_START',
               'NON_SYNONYMOUS_STOP', 'RARE_AMINO_ACID', 'SPLICE_SITE_ACCEPTOR', 'SPLICE_SITE_DONOR',
               'SPLICE_SITE_REGION', 'SPLICE_SITE_BRANCH', 'SPLICE_SITE_BRANCH_U12', 'STOP_LOST', 'START_GAINED',
               'START_LOST', 'STOP_GAINED', 'SYNONYMOUS_CODING', 'SYNONYMOUS_START', 'SYNONYMOUS_STOP', 'TRANSCRIPT',
               'REGULATION', 'UPSTREAM', 'UTR_3_PRIME', 'UTR_3_DELETED', 'UTR_5_PRIME', 'UTR_5_DELETED']
    ...
    # Extra options
    genes_in_common = forms.BooleanField(required=False,
                                         label="Show only variants present in genes from all the individuals selected",
                                         initial=True)
    ...
    exclude_segdup = forms.BooleanField(required=False, label="Exclude variants in regions of Segmental Duplications",
                                        initial=True)
