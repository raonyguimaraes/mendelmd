...
urlpatterns = [
    path('populate/', views.populate, name='genes_populate'),#inser genes from hgnc
    path('populatego/', views.populate_fromgo, name='populate_fromgo'),#insert data from gene_go_terms
    #path('populatetermsgo/', 'genes.views.populate_termsfromgo', name='populate_termsfromgo'),#insert data from go.obo
    #path('', 'genes.views.list', name='genes_list'),
    ...
    # path('geneontology/(?P<go_id>(?!creategroup|signup|signin)[\%\:\~\.\-\w]+)/', views.geneontology_view, name='geneontology_view'),
